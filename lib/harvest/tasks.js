...
module.exports = function(cmd, email, date) {
    ...
    TimeTracking.daily({of_user: user.id}, function(err, tasks) {
      ...
      if (opts.hasOwnProperty('running')) {
        ...
        if (opts.running === false && not_running.length == 0) {
          ...
          _msg = '_' + self.format_task_entry(not_running[0]) + '_';
        }
        return callback(user, [_msg]);
      }

      var tasks = entries.map(function(entry) {
        return self.format_task_entry(entry);
      });
  ...
  this.format_task_entry = function(entry) {
    var _msgs = [];
    _msgs.push(this.decical_to_hours(entry.hours));
    _msgs.push(entry.client);

    if (entry.notes && entry.notes != '') {
      _msgs.push(entry.notes);
    }
    return _msgs.join(' - ');
  }
